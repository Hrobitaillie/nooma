...
🔶 **Fragments de langue** (récompense par défaut v1 — cf. [03-game-design.md](03-game-design.md) §2.2 et [04-progression-adaptative.md](04-progression-adaptative.md) §5.3) : prévoir un **traitement visuel léger** — lettres/graphèmes en pâte à modeler, ou éclats lumineux typographiques qui viennent enrichir le « langage » de Plouma. **Pas d'asset 3D lourd par fragment** : éléments d'UI 2D / typographiques réutilisables, cohérents avec l'évolution du babillage→français de Plouma. Coût de production quasi nul, contrairement aux autocollants (1 rendu Blender chacun).
...
2. **Cartes-mondes** : une scène par biome, caméra fixe, rendu en 3-4 couches (parallax), ~2048 px, WebP. 🔶 *Précision technique (question Hugo — « comment voir la perspective bouger si c'est du web statique ? ») : les couches WebP sont plates ; la profondeur vient du **parallax par translation** — quand la caméra **panote** (déplacement latéral/vertical dans l'app), les couches défilent à des vitesses différentes (arrière-plan lent → avant-plan rapide) → illusion de relief sans 3D temps réel. En revanche une **vraie rotation de perspective** (tourner autour d'un décor isométrique) n'est **pas** possible depuis des couches plates : on se limite donc à des mouvements de caméra en **translation + zoom**, pas de rotation. Pour un vrai relief « qui tourne » il faudrait pré-rendre plusieurs angles ou passer ce décor en 3D légère embarquée (coûteux) — à trancher au Test.*
